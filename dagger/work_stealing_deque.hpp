...
@brief class to create a lock-free unbounded single-producer multiple-consumer
queue

This class implements the work-stealing queue described in the paper,
<a href="https://www.di.ens.fr/~zappa/readings/ppopp13.pdf">Correct and
Efficient Work-Stealing for Weak Memory Models</a>, and extends it to include
priority.
...
// Function: push
template <typename T>
inline void TaskQueue<T>::push(T item) {
  int64_t b = _bottom.load(std::memory_order_relaxed);
...
template <typename T>
typename TaskQueue<T>::Array* TaskQueue<T>::resize_array(Array* a,
                                                         std::int64_t b,
                                                         std::int64_t t) {
